...
# 对OCR模型同样使用snapshot_download指定路径
...
def run(explore_switch, instruction, id, difficulty):
    ...
    while True:
        ...
        output_action = inference_chat(chat_action, 'o4-mini', API_url, token)
        ...
        if memory_switch:
            ...
            output_memory = inference_chat(chat_action, 'o4-mini', API_url, token)
        ...
        if "Tap" in action:
            ...
            if explore_switch:
                print(f"The corresponding information found is:{action.split('|')[-1]}\n")
                return action.split('|')[-1]
            else:
                return 1
        if explore_switch == True and allsteps > 15:
            print("The corresponding information could not be found.\n")
            return 0
        ...
        end_time = time.time()
        print(
            f"The {allsteps}th operation, the output action is {summary}, and the total time is {round(end_time - start_time)} seconds.\n")
        ...
        if reflection_switch:
            ...
            output_reflect = inference_chat(chat_reflect, 'o4-mini', API_url, token)
            ...
            if 'A' in reflect:
                ...
                output_planning = inference_chat(chat_planning, 'o4-mini', API_url, token)
