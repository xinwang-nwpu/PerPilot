def get_action_prompt(instruction, clickable_infos, width, height, keyboard, summary_history, action_history,
    ...
    prompt += "Now you need to combine all of the above to perform just one action on the current page. You must choose one of the seven actions below(You cannot change the format of these actions when you output even a space.):\n"
    prompt += "Tap (x, y): Tap the position (x, y) in current page.\n"
    prompt += "Swipe (x1, y1), (x2, y2): Swipe from position (x1, y1) to position (x2, y2).\n"
    prompt += "Open app(app_name): Open the app named app_name.\n"
    if keyboard:
    ...
    prompt += "Back: Return to the previous page.\n"
    prompt += "Home: Return to phone home page.\n"
    if explore_switch:
        prompt += (
            "Stop: If you believe all the requirements of the user's instruction have been completed and no further action is needed, you can choose this operation to terminate the process. Then your output format is Stop|Information (this information is the core part of what the user needs,"
            "please keep only the essential part and remove any extra descriptive words) for example, Stop|Northwestern Polytechnical University, and do not output anything else.")
    else:
