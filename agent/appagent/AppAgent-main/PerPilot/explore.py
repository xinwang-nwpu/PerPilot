...
from chat import personalization_chat, home
...
def update_config_instruction(config, json_file_path, target_id):
    try:
        ...
        print(f"warning: can not find {json_file_path}")
        return config
    except json.JSONDecodeError:
        print(f"warning: can not explain {json_file_path}")
        return config
    ...
    if instruction:
        ...
        print(f"warning: can not find id {target_id}")
    ...
    while True:
        ...
        if response.startswith("Yes"):
            ...
            if len(b) != 0:
                ...
                print(f"information is enough, try to execute {instruction}")
                config["instruction"] = instruction
                config["explore_switch"] = False
                result = run(config["explore_switch"], config["instruction"], id, config["difficulty"])
                if not result:
