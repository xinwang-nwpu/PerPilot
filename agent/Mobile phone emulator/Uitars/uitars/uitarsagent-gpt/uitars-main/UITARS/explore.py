...
def update_config_instruction(config, json_file_path, target_id):
    ...
    if instruction:
        ...
        print(f"Warning: No item found with ID {target_id} in {json_file_path}")
...
def explore_solve(instruction, api_url, Token, model, ID):
    ...
    while True:
        ...
        append_to_file(ID, f"Command:{instruction}")
        append_to_file(ID, f"Personalization stage")
        response, token1 = inference_chat2(chat_history, model, api_url, Token)
        all_token += token1[2]
        response = response.replace('\n', '')
        print(f"Extract personalized elements:{response}")
        ...
        if response.startswith("Yes"):
            ...
            append_to_file(ID, f"Extracted personalized elements:{response[1:]}")
            tmp = response[1:]
            a, b = semantic_analysis(tmp, lines)
            if len(b) != 0:
                print(f"lack {b}\n")
                append_to_file(ID, f"Exploration stage")
                ...
                all_token += token2[2]
                response = response.split('\n')
                append_to_file(ID, f"Generated exploration commands:{response}")
                ...
                for i in range(min(len(b), len(temp_response))):
                    ...
                    if message != 0:
                        ...
                        print(f"Verify exploration information success or not, verify result is {response}")
                        all_token += token3[2]
                        if response.startswith("yes"):
                            f = open("personalization.txt", "a", encoding="utf-8")
                            f.write(b[i] + "|" + str(message) + "\n")
                            f.close()
                            append_to_file(ID, f"Accurately discovered information:{message}success\n\n")
                        else:
                            append_to_file(ID, f"Accurately discovered information:{message}failed\n\n")
                            errorflag = True
                        ...
                        # txt.append(b[i])
                        print(f"Failed to explore accurately information for {b[i]}\n")
                        append_to_file(ID, f"Failed to explore accurately information for {b[i]}\n\n")
                        home(config["adb_path"])
                        errorflag = True
                        break
                if errorflag:
                ...
                append_to_file(ID, f"The replaced instruction is:{instruction}")
                config["instruction"] = instruction
                ...
                if result:
                    ...
                    print("Verify accurate instruction execution stage")
                    print(response)
                    if "Success" in response:
                        append_to_file(ID,
                                       f"Verify accurate instruction execution success\nReason:{response},Verify token consumption:{token3[2]},Personalized elements:{tmp}\n")
                        append_to_file(ID, f"Personalized information extraction and instruction generation token consumption:{all_token}\n")
                        print("Verify accurate instruction execution success")
                    else:
                        append_to_file(ID, f"Verify accurate instruction execution failed\nReason:{response},Verify token consumption:{token3[2]}\n")
                        append_to_file(ID, f"Personalized information extraction and instruction generation token consumption:{all_token}\n")
                        print("Verify accurate instruction execution failed")
                else:
                    print("Accurate instruction execution failed")
                restart_emulator()
                ...
                print(f"Accurately execute instruction:{instruction}")
                append_to_file(ID, f"Accurately execute instruction:{instruction}")
                config["instruction"] = instruction
                config["explore_switch"] = False
                append_to_file(ID, f"Accurate instruction execution stage")
                agent = UITARS(config)
            ...
            if result:
                ...
                print(response)
                if "Success" in response:
                    append_to_file(ID, f"Verify accurate instruction execution success\nReason:{response},Verify token consumption:{token3[2]}\n")
                    append_to_file(ID, f"Personalized information extraction and instruction generation token consumption:{all_token}\n")
                    print("Verify accurate instruction execution success")
                else:
                    append_to_file(ID, f"Verify accurate instruction execution failed\nReason:{response},Verify token consumption:{token3[2]}\n")
