def get_action_prompt(instruction, clickable_infos, width, height, summary_history, action_history,
                      last_summary, last_action, add_info, error_flag, completed_content, memory, explore_switch):
    ...
    prompt += "swipe: Swipe from one position to another. (please check carefully whether the sliding operation is effective.)\n"
    ...
    prompt += "type: Input text in an input box.(Before you can use the type action, you have to click on the input box first.You must be in the active input box to use input operations.)\n"
    ...
    prompt += "When you use the swipe operation,your description should be sliding in which direction to where,But keep in mind that swiping on your phone is in the opposite direction, and if you want to swipe to the right, you can output and swipe left"
    prompt += "### Output Format ###\n"
    prompt += "Your output consists of the following two parts:\n"
    prompt += "### Thought ### \nThink about the requirements that have been completed and the requirements that need to be completed in the next step.\n"
    prompt += "### Action ### \nDescribe your next action based on the historical action records, the current state, and the action you choose. Use concise and natural language ( if you need to use type, output type(content='input content') and do not include any additional information outside the format).\n"
    return prompt
...
def get_process_prompt(instruction, thought_history, summary_history, action_history, completed_content, add_info):
    ...
    if len(thought_history) > 1:
        ...
        prompt += "### Completed contents ###\nUpdated completed content. Do not include any purpose of the actions, only summarize the actual completed content in the ### Historical Actions ###.\n"
